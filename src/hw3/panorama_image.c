...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    // We will have at most "an" matches, number of descriptors in a
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        float min_dist = l1_distance(a[j].data, b[0].data, temp_n);
        ...
        for (i = 0; i < bn; i++) {
          ...
          // // temp_n = b[k].n;
          float curr_dist = l1_distance(a[j].data, b[i].data, temp_n);
    ...
    // max boundary is an since that's the most matches we can possibly have
    qsort(m, an, sizeof(match), match_compare);
    for (i = 0; i < an; i++) {
      // seen is a bit array of have we seen this val yet
...
point project_point(matrix H, point p)
{
    ...
    // Have to divide by.... something...  <- resultant matrix
    c.data[0][0] = p.x;
    c.data[1][0] = p.y;
    c.data[2][0] = 1.0;
    matrix mult = matrix_mult_matrix(H, c);
    float x = mult.data[0][0] / mult.data[2][0];
    float y = mult.data[1][0] / mult.data[2][0];
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (int i = 0; i < n; i++) {
      // doint point distance since h*p is a point
      point Hp = project_point(H, m[i].p);
      float distance = point_distance(Hp, m[i].q);
      if (distance <= thresh) {
        // swap curr i match to the front
        match old_first = m[count];
        m[count] = m[i];
        m[i] = old_first;
        count++;
      }
...
{
    matrix M = make_matrix(n*2, 8);  // 8 cols
    matrix b = make_matrix(n*2, 1);  // 1 col
    ...
    for(i = 0; i < n; ++i){
        ...
        M.data[even_row][3] = 0;
        M.data[even_row][4] = 0;
        M.data[even_row][5] = 0;  // from slides
        M.data[even_row][6] = -x * xp;
        M.data[even_row][7] = -y * xp;
        ...
        M.data[odd_row][4] = y;
        M.data[odd_row][5] = 1;
        M.data[odd_row][6] = -x * yp;
        M.data[odd_row][7] = -y * yp;

        // 2 cols
        b.data[even_row][0] = xp;
        b.data[odd_row][0] = yp;
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (int i = 0; i < k; i++) {
      ...
      // homography is better than old if present (non-zero)
      if (homography.data) {
        int num_inliers = model_inliers(homography, m, n, thresh);
        if (num_inliers > best) {
          best = num_inliers;
          // redo Hb
          Hb = compute_homography(m, num_inliers);
          if (Hb.data) {
            // did we get data? update inliers if so
...
image combine_images(image a, image b, matrix H)
{
    ...
    for(k = 0; k < a.c; ++k){
        for(j = 0; j < a.h; ++j){
            for(i = 0; i < a.w; ++i){
                // TODO: fill in.
                float val = get_pixel(a, k, j, i);
                set_pixel(c, k, j - dy, i - dx, val);
            }
        }
    }
    ...
    for (int y = 0; y < h; y++) {
      for (int x = 0; x < w; x++) {
        // project from a to b
        point pt = make_point (x + dx, y + dy); // make using offset?
        point proj = project_point(H, pt);
        if (proj.x >= 0 && proj.x < b.w && proj.y >= 0 && proj.y < b.h) {
          // get all channels from proj
          for (int ch = 0; ch < b.c; ch++) {
            float interpol = bilinear_interpolate(b, ch, proj.y, proj.x);
            set_pixel(c, ch, y, x, interpol);
          }
        }
      }
    }
